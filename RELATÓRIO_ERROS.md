...
- Comportamento de carregamento
  - Problema: o estado `loading` não estava sendo atualizado corretamente.
  - Correção: adicionado `setLoading(true)` no início do `useEffect` e `setLoading(false)` no bloco `finally`.
